...
EXPORTS(void)                   simd_input__delete(simd_input* target) { delete target; }


/************* utf8_checking_state *************/

EXPORTS(void)                   utf8_checking_state__delete(utf8_checking_state* target) { delete target; }
...
EXPORTS(uint64_t)               _compute_quote_mask_u(uint64_t quote_bits) { return compute_quote_mask(quote_bits); }
EXPORTS(uint64_t)               _compute_quote_mask_u(uint64_t quote_bits) { return compute_quote_mask(quote_bits); }
EXPORTS(uint64_t)               _compute_quote_mask_u(uint64_t quote_bits) { return compute_quote_mask(quote_bits); }
EXPORTS(void)                   _check_utf8_su(simd_input<T> in, utf8_checking_state<T>& state) { check_utf8(in, state); }
EXPORTS(void)                   _check_utf8_su(simd_input<instruction_set::sse4_2> in, utf8_checking_state<instruction_set::sse4_2>& state) { check_utf8(in, state); }
EXPORTS(errorValues)            _check_utf8_errors_u(utf8_checking_state<T>& state) { return check_utf8_errors(state); }
EXPORTS(errorValues)            _check_utf8_errors_u(utf8_checking_state<instruction_set::sse4_2>& state) { return check_utf8_errors(state); }
EXPORTS(simd_input<T>)          _fill_input_u(const uint8_t* ptr) { return fill_input(ptr); }
EXPORTS(simd_input<instruction_set::sse4_2>) _fill_input_u(const uint8_t* ptr) { return fill_input(ptr); }
EXPORTS(uint64_t)               _cmp_mask_against_input_su(simd_input<T> in, uint8_t m) { return cmp_mask_against_input(in, m); }
EXPORTS(uint64_t)               _cmp_mask_against_input_su(simd_input<instruction_set::sse4_2> in, uint8_t m) { return cmp_mask_against_input(in, m); }
EXPORTS(uint64_t)               _unsigned_lteq_against_input_su(simd_input<T> in, uint8_t m) { return unsigned_lteq_against_input(in, m); }
EXPORTS(uint64_t)               _unsigned_lteq_against_input_su(simd_input<instruction_set::sse4_2> in, uint8_t m) { return unsigned_lteq_against_input(in, m); }
EXPORTS(uint64_t)               _find_odd_backslash_sequences_su(simd_input<T> in, uint64_t& prev_iter_ends_odd_backslash) { return find_odd_backslash_sequences(in, prev_iter_ends_odd_backslash); }
EXPORTS(uint64_t)               _find_quote_mask_and_bits_suuuu(simd_input<T> in, uint64_t odd_ends, uint64_t& prev_iter_inside_quote, uint64_t& quote_bits, uint64_t& error_mask) { return find_quote_mask_and_bits(in, odd_ends, prev_iter_inside_quote, quote_bits, error_mask); }
EXPORTS(void)                   _find_whitespace_and_structurals_suu(simd_input<T> in, uint64_t& whitespace, uint64_t& structurals) { find_whitespace_and_structurals(in, whitespace, structurals); }
...
EXPORTS(parse_string_helper)    _find_bs_bits_and_quote_bits_uu(const uint8_t* src, uint8_t* dst) { return find_bs_bits_and_quote_bits(src, dst); }
EXPORTS(parse_string_helper)    _find_bs_bits_and_quote_bits_uu(const uint8_t* src, uint8_t* dst) { return find_bs_bits_and_quote_bits(src, dst); }
...
EXPORTS(int)                    _json_parse_implementation_usPb(const uint8_t* buf, size_t len, ParsedJson& pj, bool reallocifneeded) { return json_parse_implementation(buf, len, pj, reallocifneeded); }
...
EXPORTS(int)                    _json_parse_cP(const char* buf, ParsedJson& pj) { return json_parse(buf, pj); }
...
EXPORTS(ParsedJson)             _build_parsed_json_c(const char* buf) { return build_parsed_json(buf); }
